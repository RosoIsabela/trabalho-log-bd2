import psycopg2 # Para psycopg2.Error e sql
from psycopg2 import sql # Para construção segura de SQL

def executa_redo(conn, operacoes):
    try:
        with conn.cursor() as cursor:
            cursor.execute("delete from clientes_em_memoria;")
            conn.commit()
            print("Tabela clientes_em_memoria limpa")
    except psycopg2.Error as erro:
        print(f"Erro ao limpar a tabela: {erro}")
        conn.rollback()
        return #Nao continua se a limpeza falha
